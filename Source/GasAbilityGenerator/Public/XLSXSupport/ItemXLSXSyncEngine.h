// GasAbilityGenerator - Item XLSX Sync Engine
 ...
 */
enum class EItemSyncStatus : uint8
{
 ...
 */
enum class EItemConflictResolution : uint8
{
 ...
 */
enum class EItemSyncValidationStatus : uint8
{
 ...
 */
struct FItemSyncEntry
{
	FGuid RowId;                                   // Stable row identifier
	EItemSyncStatus Status = EItemSyncStatus::Unchanged;
	EItemConflictResolution Resolution = EItemConflictResolution::Unresolved;

	// Row versions (nullptr if doesn't exist in that source)
	TSharedPtr<FItemTableRow> BaseRow;             // From last export snapshot
	TSharedPtr<FItemTableRow> UERow;               // Current in editor
	TSharedPtr<FItemTableRow> ExcelRow;            // From imported file
	...
	EItemSyncValidationStatus ValidationStatus = EItemSyncValidationStatus::Valid;
	...
	/** Get item name for display (from whichever version exists) */
 ...
 */
struct FItemSyncResult
{
	bool bSuccess = false;
	FString ErrorMessage;

	TArray<FItemSyncEntry> Entries;
	...
	TArray<FItemSyncEntry*> GetConflicts();

	/** Get entries that require user resolution (conflicts + delete conflicts) */
	TArray<FItemSyncEntry*> GetEntriesRequiringResolution();
};

/**
 * Merged result ready to apply
 */
struct FItemMergeResult
{
	TArray<FItemTableRow> MergedRows;    // Final rows to replace current data
...
/**
 * 3-way merge engine for Item table sync
 *
 * Usage:
 ...
 */
class GASABILITYGENERATOR_API FItemXLSXSyncEngine
{
public:
	/**
	 ...
	 */
	static FItemSyncResult CompareSources(
	 ...
	 */
	static FItemMergeResult ApplySync(const FItemSyncResult& SyncResult);
	 ...
	 */
	static void AutoResolveNonConflicts(FItemSyncResult& SyncResult);

	/**
	 * Check if all conflicts have been resolved
	 * @return true if ApplySync can be safely called
	 */
	static bool AllConflictsResolved(const FItemSyncResult& SyncResult);
...
private:
	...
	static TMap<FGuid, const FItemTableRow*> BuildRowMap(const TArray<FItemTableRow>& Rows);

	/** Determine sync status from presence and hash comparison */
	static EItemSyncStatus DetermineStatus(
