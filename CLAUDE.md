...
- Row operations: Add/Delete/Delete Branch (cascade)
- Tree traversal with depth-first sequence ordering
- CSV import/export (RFC 4180 compliant)
- Per-column filtering (text + multi-select dropdown)
- Clear All Filters button

**NPC Table Editor** (`SNPCTableEditor`)
- 17 columns in 6 groups:
  - Core Identity (5): Status, NPCName, NPCId, DisplayName, Blueprint
  - AI & Behavior (3): AbilityConfig, ActivityConfig, Schedule
  - Combat (3): LevelRange, Factions, AttackPriority
  - Vendor (2): IsVendor, ShopName
  - Items & Spawning (2): DefaultItems, SpawnerPOI
  - Meta (2): Appearance, Notes
- Sync from existing NPCDefinition assets
- Apply changes back to NPCDefinition assets
- Cell types: TextInput, AssetDropdown, MultiSelectDropdown, LevelRange, FloatSlider, Checkbox
- CSV import/export
- Save/Open table data persistence
...
**Full UI/UX comparison:** See `ClaudeContext/Handoffs/Table_Editors_Reference.md`
