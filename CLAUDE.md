...
- v4.14.1 - GAS Function Name Corrections: Fixed Blueprint compilation errors in form abilities. `ApplyGameplayEffectToSelf` on `AbilitySystemBlueprintLibrary` doesn't exist - changed to `BP_ApplyGameplayEffectToOwner` with `target_self: true` (function is on UGameplayAbility). `BP_RemoveGameplayEffectFromOwnerByClass` doesn't exist in Blueprint - changed to `BP_RemoveGameplayEffectFromOwnerWithGrantedTags` using `Father.State.Transitioning` granted tag. Removed unnecessary `GetFatherASC` nodes since `target_self: true` handles self-targeting. All 5 form abilities (Crawler, Armor, Exoskeleton, Symbiote, Engineer) updated with correct function calls and tag-based effect removal.
- v4.14 - Full Transition Automation: All 5 form abilities now include complete transition invulnerability automation in their event graphs. Each form ability applies `GE_TransitionInvulnerability` on activate and removes it (by granted tag) on end. Pattern: Event_Activate → ApplyTransitionInvuln → FormLogic; Event_End → RemoveTransitionInvuln. Standardized across GA_FatherCrawler, GA_FatherArmor, GA_FatherExoskeleton, GA_FatherSymbiote, GA_FatherEngineer.
