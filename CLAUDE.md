...
GasAbilityGenerator is an Editor plugin (v4.11) that generates UE5 assets from YAML manifest definitions and CSV dialogue data.
...
## GasAbilityGenerator Plugin (v4.11)
...
- v4.11 - Niagara Emitter Enable/Disable Controls: Two-tier emitter enable/disable system distinguishing runtime-safe operations from compile-required structural changes. **Soft Enable** (`enabled:`) sets `{Emitter}.User.Enabled` parameter - runtime toggle, no recompile needed. **Structural Enable** (`structural_enabled:`) calls `FNiagaraEmitterHandle::SetIsEnabled()` - compile-time change, triggers system recompilation only when state actually changes. **Sentinel Pattern:** New fields `bHasEnabled`, `bHasStructuralEnabled` distinguish "not specified" from "explicitly set to default" - prevents unintended operations from default values. **Compile Doctrine:** Removed unconditional `RequestCompile()` at end of Niagara generation; now compiles ONLY for structural emitter changes (SetIsEnabled returns true). **Safety Gate:** Added `IsReadyToRun()` check before save with diagnostic message including `structural_changes=true/false` for debugging. **Hash Updates:** `FManifestNiagaraEmitterOverride::ComputeHash()` now includes all four fields (bEnabled, bHasEnabled, bStructuralEnabled, bHasStructuralEnabled) for proper regen/diff detection. See `ClaudeContext/Handoffs/VFX_System_Reference.md`.
- v4.10 - Material Expression Validation with 6 Guardrails: Hallucination-proof validation system for material expressions. New types: EMaterialExprValidationCode enum (11 error codes, 10 implemented + 1 reserved), FMaterialExprDiagnostic struct with context path/user input/suggested fix, FMaterialExprValidationResult with 6 guardrail helpers. **6 Guardrails:** (1) Quote Strip Before Trim - handle YAML quoted strings, (2) Case Hint on User Path - show original input in warnings, (3) ContextPath Root Convention - `/Materials/{Name}/{Section}` format, (4) Normalize Once Reuse Everywhere - single normalization pass, (5) Field-Specific Normalizers - NormalizeKey/NormalizeScalar/NormalizePath, (6) Stable Diagnostic Ordering - sort by path/code/message for CI. **3-Pass Algorithm:** Pass 1 validates expressions (type, switch keys, function paths, required default input, function instance blocking), Pass 2 validates connections, Pass 3 warns on unused expressions. **Validations:** E_UNKNOWN_EXPRESSION_TYPE (97 known types), E_UNKNOWN_SWITCH_KEY, E_FUNCTION_NOT_FOUND, E_FUNCTION_INSTANCE_BLOCKED (blocks _Inst paths), E_MISSING_REQUIRED_INPUT (switch default required), E_DUPLICATE_EXPRESSION_ID, E_EXPRESSION_REFERENCE_INVALID, W_DEPRECATED_SWITCH_KEY (es2/sm4), W_FUNCTION_PATH_NORMALIZED, W_EXPRESSION_UNUSED. Both FMaterialGenerator and FMaterialFunctionGenerator validate before generation. See `ClaudeContext/Handoffs/VFX_System_Reference.md`.
