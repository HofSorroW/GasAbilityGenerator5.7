...
- v4.10 - Material Expression Validation with 6 Guardrails: Hallucination-proof validation system for material expressions. New types: EMaterialExprValidationCode enum (11 error codes), FMaterialExprDiagnostic struct with context path/user input/suggested fix, FMaterialExprValidationResult with 6 guardrail helpers. **6 Guardrails:** (1) Quote Strip Before Trim - handle YAML quoted strings, (2) Case Hint on User Path - show original input in warnings, (3) ContextPath Root Convention - `/Materials/{Name}/{Section}` format, (4) Normalize Once Reuse Everywhere - single normalization pass, (5) Field-Specific Normalizers - NormalizeKey/NormalizeScalar/NormalizePath, (6) Stable Diagnostic Ordering - sort by path/code/message for CI. **3-Pass Algorithm:** Pass 1 validates expressions (type, switch keys, function paths), Pass 2 validates connections, Pass 3 warns on unused expressions. 60+ known expression types in exhaustive KnownExpressionTypes set. Both FMaterialGenerator and FMaterialFunctionGenerator now validate before generation. See `ClaudeContext/Handoffs/VFX_System_Reference.md`.
