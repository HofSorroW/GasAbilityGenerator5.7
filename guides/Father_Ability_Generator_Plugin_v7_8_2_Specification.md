...
|-----------|-------|---------|
| Window Widget | SSpiderAbilityGeneratorWindow | Main UI container |
...
| ManifestData | FFatherManifestData | Empty | Parsed manifest content |
...
|--------|----------|---------------|
| FFatherCurveKeyDefinition | Time | 0.0f |
| FFatherCurveKeyDefinition | Value | 0.0f |
| FFatherModifierDefinition | ScalableFloatValue | 0.0f |
| FFatherGameplayEffectDefinition | DurationMagnitude | 0.0f |
| FFatherGameplayEffectDefinition | bExecutePeriodicOnApplication | false |
| FFatherGameplayEffectDefinition | StackLimitCount | 0 |
| FFatherItemCollectionEntry | Quantity | 1 |
| FFatherActivityConfigDefinition | RequestInterval | 0.5f |
| FFatherBlackboardKeyDefinition | bInstanceSynced | false |
| FFatherWidgetVariableDefinition | bInstanceEditable | false |
| FFatherWidgetVariableDefinition | bExposeOnSpawn | false |
| FFatherActorVariableDefinition | bReplicated | false |
| FFatherActorVariableDefinition | bInstanceEditable | false |
| FFatherAssetStateEntry | bExists | false |
...
| 2. actor_blueprints | BP_Test1, BP_Test2 | 2 actors only |
| 3. widget_blueprints | WBP_Test1 | 1 widget only |
...
**CRITICAL (v2.5.4+): After MakeLinkTo(), call NotifyPinConnectionListChanged on BOTH nodes:**

```cpp
// After making any connection with MakeLinkTo():
FromNode->NotifyPinConnectionListChanged(FromPin);
ToNode->NotifyPinConnectionListChanged(ToPin);
```

This is REQUIRED for DynamicCast nodes to recognize the connected type. Without this call, the compiler reports: "The type of Object is undetermined."
...
| Branch | then, else | Exec, Condition |
...
**Note (v2.5.4+):** Branch node (UK2Node_IfThenElse) uses lowercase `then`/`else` internally, NOT `True`/`False` as displayed in the Blueprint editor. Always use `then`/`else` when referencing Branch output pins programmatically.
...
### v7.8.2 Plugin Bug Fixes (v2.1.0 - v2.1.5)
...
| Component | Issue | Fix |
|-----------|-------|-----|
| FFatherManifestData | No GetTotalAssetCount() function | Added function that sums all parsed arrays |
...
| Fixed struct forward reference | Moved FFatherActorVariableDefinition before dependent structs |
...
| File | Changes |
|------|---------|
| SpiderAbilityGeneratorTypes.h | Added EGenerationStatus, FGenerationResult, FGenerationSummary |
| SpiderAbilityGeneratorGenerators.h | Updated all generators to return FGenerationResult |
| SpiderAbilityGeneratorGenerators.cpp | Implemented CheckExistsAndPopulateResult helper |
| SpiderAbilityGeneratorWindow.h | Added bIsGenerating guard flag |
| SpiderAbilityGeneratorWindow.cpp | Check guard in button handlers, use FGenerationSummary |
| SpiderAbilityGeneratorParser.cpp | Fixed ShouldExitSection logic |
